...
const DynamicForm = require('./formDataDynamic.model');
...
module.exports = {
    ...
    async updatePrivatePublicStatus(req, res) {
        try {
            var form = {
                form_public_link: req.body.form_public_link
            };
    ...
    // Finding the value for Data for private or public
    async findPrivatePublicForm(req, res) {
        try {
            const { id } = req.params;
            const form = await Form.find({ '_id': id, 'form_public_link': true });
            // console.log(form);
            // return res.json(form);
            if (form && form != "") {
